...
static void ggml_compute_forward_argsort_f32(
    ...
    for (int64_t i = ith; i < nr; i += nth) {  // already multi-thread
        int32_t * dst_data = (int32_t *)((char *) dst->data + i*nb1); // result is stored in dst_data
        const float * src_data = (float *)((char *) src0->data + i*nb01); // char* is for byte-level offset
    ...
    //todo:改成load优先  strcut struct experts_pool* = global_expert_pool[UP_TYPE]  #define  0 # define GATE_TYPE 1  #define DOWN_TYPE 2
    
}
void ggml_compute_forward_argsort_load_first(
    const ggml_compute_params * params,
    ggml_tensor * dst) {
