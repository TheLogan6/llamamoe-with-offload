...
struct experts_pool* global_expert_pool[3];
// backend buffer type
...
int init_expert(struct expert_cache* target,char *dir_path, char * name,bool ifload, int expert_id){
    if (target == NULL){
        return -1;
    }
    ...
    // 你需要做的是 target->name = 在（name  中间插入  expert_id），  例如，这里的name = blk.31.ffn_up_exps.weight,  改成，blk.31.ffn_up_exps.{expert_id}.weight
...
enum ggml_status ggml_backend_tensor_alloc(ggml_backend_buffer_t buffer, struct ggml_tensor * tensor, void * addr, char* experts_path, int32_t load_experts_number) {
    ...
    // 检查tensor名称是否包含"experts"
    if (tensor->name != NULL && strstr(tensor->name, "exps") != NULL) {
        // 为专家指针数组分配内存
        char* dir_path = experts_path;
        
        tensor->experts = (struct expert_cache**)malloc(tensor->ne[2] * sizeof(struct expert_cache*));

        int type_id = ggml_get_experts_type_from_name(tensor);
        int layerid = ggml_get_layerid_from_name(tensor);

        for (int cur_a = 0; cur_a < tensor->ne[2]; cur_a++) {
            tensor->experts[cur_a] = (struct expert_cache*)malloc(sizeof(struct expert_cache));
            tensor->experts[cur_a]->pool = global_expert_pool[type_id];
            
            global_expert_pool[type_id]->experts[cur_a + layerid * global_expert_pool[type_id]->n_experts] = tensor->experts[cur_a];
        }
