...
size_t ggml_tensor_overhead(void) {
    return GGML_OBJECT_SIZE + GGML_TENSOR_SIZE; // 32 + 336
}
...
struct ggml_tensor * ggml_get_rows(
        ...
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    GGML_ASSERT(a->ne[2] == b->ne[1]); //  a=2048*151936
    GGML_ASSERT(a->ne[3] == b->ne[2]);  // b=512*1*1*1
...
// ggml_flash_attn_ext
struct ggml_tensor * ggml_argsort_loadfirst(
    struct ggml_context  * ctx,
    struct ggml_tensor   * a,
    enum ggml_sort_order   order,
    int layer_id) {
...
struct ggml_tensor * ggml_top_k_loadfirst(
    struct ggml_context * ctx,
    struct ggml_tensor  * a,
    int                   k,
    int layer_id) {
...
struct ggml_tensor * result = ggml_argsort_loadfirst(ctx, a, GGML_SORT_ORDER_DESC,layer_id);
...
int ggml_get_experts_type_from_name(struct ggml_tensor * tensor){
    // tensor->name: case： blk.17.ffn_gate_exps.{}.weight
    char *start = strchr(tensor->name, '_');
    // int number_length = end - start - 1;
