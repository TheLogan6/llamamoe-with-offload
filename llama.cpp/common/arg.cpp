...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"-ex_path", "--experts_path"}, "FNAME",
            "the experts path stored in disk",
        [](common_params & params, const std::string & value) {
            params.model.experts_path = new char[value.length() + 1];
            std::strcpy(params.model.experts_path, value.c_str());
            // global_experts_path = value;
        }
    ).set_env("LLAMA_ARG_MODEL_EXPERTS"));

    add_opt(common_arg(
        {"-ex_n", "--load_experts_number"}, "N",
            string_format("the number of experts to load (default: %d, which means all experts will be loaded)", params.model.load_experts_number),
        [](common_params & params, int32_t value) {
            params.model.load_experts_number = value;
            // global_load_experts_number = value;
        }
    ).set_env("LLAMA_ARG_LOAD_EXPERTS_NUMBER"));
