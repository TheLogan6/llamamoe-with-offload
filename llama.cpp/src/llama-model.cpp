...
static bool weight_buft_supported(const llama_hparams & hparams, ggml_tensor * w, ggml_op op, ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev) {
    ...
    switch (op) {
        ...
        case GGML_OP_MUL_MAT: // 为什么全是硬编码512
            {
...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    // hparams.n_expert_used = 6; // hard encoding
}
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        for(int i = 0; i < 3; i++){
            global_expert_pool[i] = (struct experts_pool*)malloc(sizeof(struct experts_pool));
            global_expert_pool[i]->n_layer = n_layer;
            global_expert_pool[i]->n_experts = n_expert;
...
llama_model_params llama_model_default_params() {
    llama_model_params result = {
        ...
        /*   experts_path         */      nullptr,
        /*   load_experts_number */       128 
    };
...
void llama_model_tensor_free(llama_model * model){

    std::vector<std::pair<std::string, struct ggml_tensor *>> model_tensor = model->tensors_by_name;
    
    for (const auto& pair : model_tensor) {
        struct ggml_tensor * tensor = pair.second;
        if (tensor != NULL && tensor->data != NULL) {
            if(strstr(tensor->name, "exps") != NULL){
                for(int32_t i = 0; i < model->hparams.n_expert; i++){
                    if(tensor->experts[i]->data != NULL){
                        free(tensor->experts[i]->data);
                    }
                }
            }
            else{
                free(tensor->data);
                tensor->data = NULL; // 防止二次释放
            }
        }
    }
}

void llama_model_global_free(){
    for(int i = 0; i < 3; i++){
        if(global_expert_pool[i]->experts != NULL){
            free(global_expert_pool[i]->experts);
        }
        free(global_expert_pool[i]);
    }
    return ;
}
